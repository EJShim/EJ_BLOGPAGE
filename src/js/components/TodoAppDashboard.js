...
export default class TodoAppDashboard extends Component {
  ...
  render() {
    ...
    const tasks = this.state.tasks.map((task, index) => {
      ...
      return (
        <ListItem key={`task_${index}`} justify='between'
          ...
          <DeleteTaskButton task={task} onDelete={this._onRequestForDelete.bind(this)} />
        </ListItem>
      );
    }, this);
    ...
    if (this.state.addTask) {
      addTask = (
        <TodoAddTaskForm onClose={this._onRequestForAddClose.bind(this)}
          onSubmit={this._onAddTask.bind(this)} />
      );
    }
        ...
        <Box direction='row'>
          <Box basis='1/3' align='center'>
            <Meter series={series} type='circle' label={false}
              onActive={this._onMeterActive.bind(this)} />
